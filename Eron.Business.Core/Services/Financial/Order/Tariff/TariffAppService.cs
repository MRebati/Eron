...
namespace Eron.Business.Core.Services.Financial.Order.Tariff
{
    public class TariffAppService : ManagementSystemService, ITariffAppService
    {
        ...
        {
            var categoryExists = await UnitOfWork.TariffCategoryRepository.GetExistsAsync(x => x.Id == categoryId);
            if (categoryExists)
            {
                var entityList = await UnitOfWork.TariffRepository.GetQueryable(x => x.TariffCategoryId == categoryId && !x.IsDeleted).ToListAsync();
