...
namespace Eron.DataAccess.EntityFramework.Repositories.Financial.Shop
{
    public class InvoiceRepository : Repository<Invoice>, IInvoiceRepository
    {
        ...
        public int GetInvoiceAmount(long invoiceId)
        {
            int finalPrice = 0;
            var result = (from invoiceItem in _invoiceItemRepository.GetQueryable()
            ...
            foreach (var item in result)
            {
                if (item.productPrice != null)
                    finalPrice += (int)item.productPrice.Price;
                if (item.tariffPrice != null)
                    finalPrice += (int)item.tariffPrice.Price;
            }

            return finalPrice;
        }

        public async Task<int> GetInvoiceAmountAsync(long invoiceId)
        {
            int finalPrice = 0;
